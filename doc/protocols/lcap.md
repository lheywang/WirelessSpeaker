...
* `0x00`: **HOST** (The FPGA/NIOS V)
* `0x10`: **SAT_WIFI** (Wi-Fi 5/6 Module)
* `0x20`: **SAT_BLE** (Bluetooth LE Module)
* `0x30`: **SAT_USB** (USB/SPDIF Module)
* `0x40`: **SAT_HDMI** (HDMI Bridge)
...
## 6. Implementation Reference (C Structs)

```c
// LACP Header Definition
typedef struct __attribute__((packed)) {
    uint8_t sync;       // Always 0x55
    uint8_t dev_id;     // Sender ID
    uint8_t seq_id;     // Message Counter
    uint8_t cmd;        // Command OpCode
    uint8_t len;        // Payload Length
} lacp_header_t;

// Standard Formats
#define LACP_DEV_HOST     0x00
#define LACP_DEV_WIFI     0x10
#define LACP_DEV_BLE      0x20

#define LACP_CMD_SYS_ACK  0x01
#define LACP_CMD_AUD_MUTE 0x11
